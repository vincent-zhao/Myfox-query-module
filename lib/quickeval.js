		...
		while(true){
			...
			if(pos < 0){
				...
				if(left > 0 && _self.token[left-1]["type"] == Lexter.types.FUNCTION){
					...
					index++;
				}else if(_self.token[left]["mark"] == null || _self.token[left]["mark"] == -1 || _self.token[left]["mark"] == 0){
					_self.stack.push({
						"call" : "STATIC",
						"args" : [_self.get_token_var(find ? left+1 : left)]
					});
					index++;
				}
...
QuickEval.prototype.get_token_args = function(pos){
	...
	if(token["type"] == Lexter.types.OPERATOR){
		return [_self.get_token_var(pos),_self.get_token_var(pos-1),_self.get_token_var(pos+1)];
	}
