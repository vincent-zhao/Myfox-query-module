 ...
 * @return void
 */
function findNodes(table,field,cb){
  var route = routeValue(table,field);
  var get = lcache.get(route);
  if(get !== false){
    cb(null,get); 
  }else{
    var sql = "SELECT hosts_list,real_table,addtime FROM $$v1 WHERE route_sign = $$v2 AND table_name = $$v3 AND route_text=$$v4 AND route_flag >= 300 AND route_flag < 400";
    ...
    dataloader.query(sql,function(err,data){
      if(err){
        cb(err,data);
        return;
      }
